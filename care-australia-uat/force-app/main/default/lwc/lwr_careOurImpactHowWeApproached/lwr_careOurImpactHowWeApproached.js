...
export default class Lwr_careOurImpactHowWeApproached extends LightningElement {
    @track screenWidth;
    @track screenHeight;
    title3;
    approachbody;
   ...
   //evidencebody;
    ...
    wiredCustomSectionMetaDataRecords({ data, error }) {
        if (data && data.length > 0) {
            console.log('@@@ Inside wire ourimpactpage 4th section');
            console.log('@@@length:', data.length);
            console.log('@@@ data :', data);
 
            for (let i = 0; i < data.length; i++) {
                if (data[i].MasterLabel == "Empowerment") {
                    this.val1 = data[i].Value_1__c;
                    this.val2 = data[i].Value_2__c;
                }
            }
            console.log('@@@ Empowerment in val1:', this.val1);
            ...
            if (r != undefined) {
                for (var val of r.items) {
                    if (val.type == 'CareAustraliaSite' && val.contentNodes.Tag != undefined) {
                        ...
                        if (val.contentNodes.Tag.value == 'Howweapproached') {
                           
                           /*var title = val.contentNodes.Title.value;
                            this.title3 = title
                            console.log('@@@ Title part :', this.title3);*/

                            this.image1 = this.imgUrl+val.contentNodes.Image.url;
                            console.log('@@@ image part :', this.image1);
                            ...
                            body = body.replaceAll('/', '');
                            body = body.replaceAll('&#39;', '\'');*/
                            this.approachbody = body;
                            console.log('@@@ approach body: ', this.approachbody);
                        }
                    }
                }  
