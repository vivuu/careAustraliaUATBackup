...
export default class Lwr_careOurImpactTheEvidence extends LightningElement {
    @track screenWidth;
    @track screenHeight;
    evidencebody;
    image2;
    ...
    wiredCustomSectionMetaDataRecords({ data, error }) {
        if (data && data.length > 0) {
            console.log('@@@ Inside wire ourimpactpage 5th section');
            console.log('@@@length:', data.length);
            console.log('@@@ data :', data);
 
            for (let i = 0; i < data.length; i++) {
                if (data[i].MasterLabel == "The evidence") {
                    this.val1 = data[i].Value_1__c;
                    this.val2 = data[i].Value_2__c;
                }
            }
            console.log('@@@ The evidence in val1:', this.val1);
            ...
            if (r != undefined) {
                for (var val of r.items) {
                    if (val.type == 'CareAustraliaSite' && val.contentNodes.Tag != undefined) {
                        ...
                        if (val.contentNodes.Tag.value == 'Theevidencelabel') {
                           
                            /*var title = val.contentNodes.Title.value;
                            this.title4 = title
                            console.log('@@@ Title part :', this.title4);*/

                            this.image2 = this.imgUrl+val.contentNodes.Image.url;
                            console.log('@@@ image part :', this.image2);
                            ...
                            this.evidencebody = body;
                            console.log('@@@ evidence body: ', this.evidencebody);
                        }
                    }
                } 
